// PhishGuard Training Script 1 â€” URL Pattern Training
// HOW TO USE:
...
chrome.storage.local.get('weights', async function(data) {
  ...
  PHISHING_URLS.forEach(url => {
    ...
    if (score >= 3) {
      correct++;
      console.log('âœ… Caught phishing:', url, '| score:', score.toFixed(1));
    } else {
      Object.keys(weights).forEach(k => { if (k !== 'blacklist') weights[k] = Math.min(5, weights[k] + 0.3); });
      console.log('âŒ Missed phishing:', url, '| score:', score.toFixed(1), 'â€” increased weights');
    }
  });
  ...
  SAFE_URLS.forEach(url => {
    ...
    if (score === 0) {
      correct++;
      console.log('âœ… Safe site clear:', url);
    } else {
      Object.keys(weights).forEach(k => { if (k !== 'blacklist') weights[k] = Math.max(0.1, weights[k] - 0.2); });
      console.log('âš ï¸ False positive:', url, '| score:', score.toFixed(1), 'â€” decreased weights');
    }
  });

  await chrome.storage.local.set({ weights });
  console.log('========================================');
  console.log('Training complete. Accuracy:', correct + '/' + total, '(' + Math.round(correct/total*100) + '%)');
