...
// HOW TO USE:
//   1. Click PhishGuard icon → right-click → Inspect → Console
//   2. Paste this entire script and press Enter

const CONFIG = {
  DANGER_THRESHOLD: 3,
  TRAIN_SPLIT: 0.8,
  MAX_PHISHING_URLS: 500,
  MAX_SAFE_URLS: 500,
  LEARNING_RATE_UP: 0.3,
...
const SAFE_SITES = [
  ...
  'https://netflix.com','https://reddit.com','https://wikipedia.org',
  'https://github.com','https://stackoverflow.com','https://twitch.tv',
  'https://discord.com','https://spotify.com','https://paypal.com',
  'https://adobe.com','https://dropbox.com','https://slack.com',
  'https://zoom.us','https://cloudflare.com','https://mozilla.org',
  'https://python.org','https://nodejs.org','https://docker.com',
  'https://bbc.com','https://cnn.com','https://nytimes.com',
  'https://reuters.com','https://bloomberg.com','https://wsj.com',
  'https://chase.com','https://bankofamerica.com','https://wellsfargo.com',
  'https://citibank.com','https://capitalone.com','https://visa.com',
  'https://mastercard.com','https://americanexpress.com','https://ally.com',
  'https://techcrunch.com','https://wired.com','https://arstechnica.com',
  'https://elastic.co','https://grafana.com','https://prometheus.io',
...
async function bulkTrain() {
  ...
  console.log('==============================================');
  console.log('\n[1/5] Fetching live threat feeds...');

  let phishingUrls = [];
  try {
    let r1 = await fetch('https://openphish.com/feed.txt');
    let t1 = await r1.text();
    let openphish = t1.split('\n').filter(u => u.trim() !== '');
    console.log('OpenPhish:', openphish.length, 'URLs');
    phishingUrls.push(...openphish);
  } catch(e) { console.log('OpenPhish failed:', e.message); }

  try {
    let r2 = await fetch('https://urlhaus.abuse.ch/downloads/text/');
    let t2 = await r2.text();
    let urlhaus = t2.split('\n').filter(u => u.trim() !== '' && !u.startsWith('#'));
    console.log('URLhaus:', urlhaus.length, 'URLs');
    phishingUrls.push(...urlhaus);
  } catch(e) { console.log('URLhaus failed:', e.message); }

  phishingUrls = [...new Set(phishingUrls)].slice(0, CONFIG.MAX_PHISHING_URLS);
  let safeUrls = SAFE_SITES.slice(0, CONFIG.MAX_SAFE_URLS);
  console.log('\nTotal phishing URLs:', phishingUrls.length);
  console.log('Total safe URLs:', safeUrls.length);

  console.log('\n[2/5] Splitting data 80/20 train/test...');
  function splitData(arr) {
    let shuffled = [...arr].sort(() => Math.random() - 0.5);
    let split = Math.floor(shuffled.length * CONFIG.TRAIN_SPLIT);
    return { train: shuffled.slice(0, split), test: shuffled.slice(split) };
  }
  let phishSplit = splitData(phishingUrls);
  let safeSplit = splitData(safeUrls);
  console.log('Training set:', phishSplit.train.length, 'phishing +', safeSplit.train.length, 'safe');
  console.log('Test set:', phishSplit.test.length, 'phishing +', safeSplit.test.length, 'safe');
  ...
  console.log('\n[3/5] Starting weights:', JSON.stringify(weights));
  ...
  for (let epoch = 0; epoch < CONFIG.EPOCHS; epoch++) {
    ...
    phishSplit.train.forEach(url => {
      let score = scoreUrl(url, weights); total++;
      if (score >= CONFIG.DANGER_THRESHOLD) { correct++; }
      else { Object.keys(weights).forEach(k => { if (k !== 'blacklist') weights[k] = Math.min(5, weights[k] + CONFIG.LEARNING_RATE_UP); }); }
    });
    safeSplit.train.forEach(url => {
      let score = scoreUrl(url, weights); total++;
      if (score === 0) { correct++; }
      else { Object.keys(weights).forEach(k => { if (k !== 'blacklist') weights[k] = Math.max(0.1, weights[k] - CONFIG.LEARNING_RATE_DOWN); }); }
    });
    let testResults = [
      ...phishSplit.test.map(url => ({ isPhishing:true,  detected: scoreUrl(url,weights) >= CONFIG.DANGER_THRESHOLD })),
      ...safeSplit.test.map(url  => ({ isPhishing:false, detected: scoreUrl(url,weights) >= CONFIG.DANGER_THRESHOLD })),
    ];
    let m = calculateMetrics(testResults);
    console.log(`Epoch ${epoch+1}/${CONFIG.EPOCHS} — Train: ${Math.round(correct/total*100)}% | F1: ${m.f1.toFixed(3)} | Precision: ${m.precision.toFixed(3)} | Recall: ${m.recall.toFixed(3)}`);
    if (m.f1 > bestF1) { bestF1 = m.f1; bestWeights = {...weights}; }
  }

  console.log('\n[5/5] Final evaluation...');
  let finalResults = [
    ...phishSplit.test.map(url => ({ isPhishing:true,  detected: scoreUrl(url,bestWeights) >= CONFIG.DANGER_THRESHOLD })),
    ...safeSplit.test.map(url  => ({ isPhishing:false, detected: scoreUrl(url,bestWeights) >= CONFIG.DANGER_THRESHOLD })),
  ];
  ...
  console.log('  True Positives  (caught phishing): ', fm.tp);
  console.log('  False Positives (false alarms):    ', fm.fp);
  console.log('  True Negatives  (safe sites clear):', fm.tn);
  console.log('  False Negatives (missed phishing): ', fm.fn);
  console.log('\nMetrics:');
  console.log('  Accuracy: ', (fm.accuracy*100).toFixed(1)+'%');
  console.log('  Precision:', (fm.precision*100).toFixed(1)+'% (low false positives)');
  console.log('  Recall:   ', (fm.recall*100).toFixed(1)+'% (threats caught)');
  console.log('  F1 Score: ', fm.f1.toFixed(3), '(overall detector quality)');
  console.log('\nBest weights saved:', JSON.stringify(bestWeights));
  console.log('==============================================');
}
