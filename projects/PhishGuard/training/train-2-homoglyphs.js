// PhishGuard Training Script 2 — Homoglyph & Brand Spoofing
// Trains detection of digit substitution: paypa1, g00gle, micros0ft
// HOW TO USE: right-click PhishGuard popup → Inspect → Console → paste

const HOMOGLYPH_PHISHING = [
  'https://paypa1.com/login', 'https://g00gle-secure.com/verify',
  'https://micros0ft-support.com/account', 'https://app1e-id.com/signin',
  'https://arnazon-deals.com/account', 'https://faceb00k.com/login',
  'https://lnstagram-verify.com/account', 'https://tw1tter-support.com/verify',
  'https://disc0rd-nitro.com/claim', 'https://steamc0mmunity.com/login',
  'https://paypai-secure.com/verify', 'https://googIe.com/signin',
  'https://micosoft.com/support', 'https://arnaz0n-prime.com/verify',
  'https://app1e-support.com/id/verify',
];

const SAFE_URLS = [
  'https://paypal.com', 'https://google.com', 'https://microsoft.com',
  'https://apple.com', 'https://amazon.com', 'https://facebook.com',
  'https://instagram.com', 'https://twitter.com', 'https://discord.com',
...
chrome.storage.local.get('weights', async function(data) {
  ...
  HOMOGLYPH_PHISHING.forEach(url => {
    ...
    else { weights.homoglyphs = Math.min(5, weights.homoglyphs + 0.4); console.log('❌ Missed:', url, '| score:', score.toFixed(1)); }
  });
  SAFE_URLS.forEach(url => {
    let score = scoreUrl(url, weights); total++;
    if (score === 0) { correct++; } else { weights.homoglyphs = Math.max(0.1, weights.homoglyphs - 0.2); console.log('⚠️ False positive:', url); }
  });
